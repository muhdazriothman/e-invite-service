...
import { DateValidator, ParseDateParams } from '@common/utils/date';
import { createMock } from '@test/utils/mocks';

describe('@flight/application/use-cases/search-flight', () => {
  let searchFlightUseCase: SearchFlightUseCase;
  ...
  beforeEach(async () => {
    ...
    shouldApplyDiscountSpy = jest.spyOn(
      SearchFlightUseCase,
      'shouldApplyDiscount',
    );
    ...
    searchFlightUseCase = module.get<SearchFlightUseCase>(SearchFlightUseCase);
  });
  ...
  describe('#execute', () => {
    ...
    beforeEach(() => {
      ...
      sortByItineraryPriceSpy = jest.spyOn(
        SearchFlightUseCase,
        'sortFlightData',
      );
    });
    ...
    function assertParseDate(callIndex: number, params: ParseDateParams) {
      expect(DateValidator.parseDate).toHaveBeenNthCalledWith(
        callIndex,
        params.date,
        params.format,
      );
    ...
    function assertShouldApplyDiscount(params: ShouldApplyDiscountParams) {
      expect(SearchFlightUseCase.shouldApplyDiscount).toHaveBeenCalledWith(
        params,
      );
    }
    ...
    it('should successfully search flights with valid dates', async () => {
      ...
      const result = await searchFlightUseCase.execute(dto);
    ...
    it('should apply discount when trip duration is more than 10 days', async () => {
      ...
      const result = await searchFlightUseCase.execute(longTripQuery);
    ...
    it('should throw BadGatewayException when flight data service is not available', async () => {
      mockFlightService.searchFlight.mockRejectedValue(
        new Error('Flight data service is not available'),
      );

      await expect(searchFlightUseCase.execute(dto)).rejects.toThrow(
        new BadGatewayException('Flight data service is not available'),
      );
    ...
    it('should throw BadRequestException when departure date is not valid', async () => {
      ...
      await expect(
        searchFlightUseCase.execute(invalidDateQuery),
      ).rejects.toThrow(new BadRequestException('Invalid date'));
    ...
    it('should throw BadRequestException when date validation fails', async () => {
      ...
      await expect(searchFlightUseCase.execute(pastDateQuery)).rejects.toThrow(
  ...
  describe('#validateDate', () => {
    ...
    it('should throw BadRequestException for past departure date', () => {
      ...
      expect(isPastDateSpy).toHaveBeenCalledWith(
        pastDepartureDate.departureDate,
      );
  ...
  describe('#sortFlightData', () => {
    ...
    it('should sort flights by price with ascending order', () => {
      ...
      expect(sortedFlights).toEqual([flights[0], flights[2], flights[1]]);
    });
    ...
    it('should sort flights by price with descending order', () => {
      ...
      expect(sortedFlights).toEqual([flights[1], flights[2], flights[0]]);
    });
  });
