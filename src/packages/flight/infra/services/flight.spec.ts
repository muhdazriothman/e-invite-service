...
describe('@flight/infra/services/flight', () => {
  ...
  beforeEach(async () => {
    ...
    const flightMapperMock = createMock<FlightMapper>();
    ...
    flightMapper = module.get(FlightMapper);
  });
  ...
  describe('#searchFlight', () => {
    ...
    it('should return flights when API call is successful', async () => {
      ...
      const mockFlights = [
        new Flight({
          ...
          priceAfterDiscountFormatted: '$200',
          legs: [],
        }),
        new Flight({
          id: 'flight-2',
          ...
          priceAfterDiscountFormatted: '$300',
          legs: [],
        }),
      ];
    ...
    it('should handle errors from the API by throwing an exception', async () => {
      ...
      await expect(service.searchFlight(searchParams)).rejects.toThrow(
        'Failed to search flights: API error',
      );
    });
  });
});
