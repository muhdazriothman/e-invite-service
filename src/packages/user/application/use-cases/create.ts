...
export class CreateUserUseCase {
    ...
    async execute(createUserDto: CreateUserDto): Promise<User> {
        ...
        const paymentRecord = await this.paymentRepository.findById(createUserDto.paymentId);
        if (!paymentRecord) {
            throw new BadRequestException('Payment record not found');
        }

        if (paymentRecord.status !== PaymentStatus.VERIFIED || paymentRecord.isDeleted) {
            throw new BadRequestException('Payment record is not available for user creation');
        }
