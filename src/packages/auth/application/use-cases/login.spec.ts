...
describe('@auth/application/use-cases/login', () => {
    ...
    describe('execute', () => {
        ...
        it('should return a JWT token when credentials are valid', async () => {
            ...
            expect(hashService.compare).toHaveBeenCalledWith(loginDto.password, user.passwordHash);
        ...
        it('should return a JWT token for admin user when credentials are valid', async () => {
            ...
            expect(userRepository.findByEmail).toHaveBeenCalledWith(loginDto.email);
            expect(hashService.compare).toHaveBeenCalledWith(loginDto.password, adminUser.passwordHash);
            expect(jwtService.sign).toHaveBeenCalledWith({
                sub: adminUser.id,
        ...
        it('should throw UnauthorizedException when user is deleted', async () => {
            ...
            await expect(useCase.execute(loginDto)).rejects.toThrow(UnauthorizedException);
            await expect(useCase.execute(loginDto)).rejects.toThrow('Account has been deactivated');
        ...
        it('should handle hash service errors gracefully', async () => {
            ...
            await expect(useCase.execute(loginDto)).rejects.toThrow('Hash service error');
        ...
        it('should handle JWT service errors gracefully', async () => {
            ...
            await expect(useCase.execute(loginDto)).rejects.toThrow('JWT service error');
