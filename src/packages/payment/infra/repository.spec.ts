...
describe('@payment/infra/repository', () => {
  ...
  beforeAll(async() => {
    ...
    paymentModel = module.get<Model<PaymentMongoDocument>>(
  ...
  describe('#create', () => {
    it('should create a new payment successfully', async() => {
      ...
      const savedPayment = await paymentModel
        .findOne({ reference: 'PAY-001' })
        .lean();
      expect(savedPayment).toBeDefined();
      expect(savedPayment?.amount).toBe(25.0);
    ...
    it('should create multiple payments with different data', async() => {
      ...
      expect(result1.amount).toBe(10.0);
      expect(result2.amount).toBe(25.0);
  ...
  describe('#delete', () => {
    ...
    it('should return false when payment does not exist', async() => {
      const result = await paymentRepository.delete(
        new Types.ObjectId().toString(),
      );
