...
export class CreatePaymentUseCase {
    ...
    async execute(createPaymentDto: CreatePaymentDto, createdBy: string): Promise<Payment> {
        const existingPayment = await this.paymentRepository.findByReference(createPaymentDto.reference);
        if (existingPayment) {
