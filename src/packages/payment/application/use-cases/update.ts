...
import { Payment, PaymentStatus } from '@payment/domain/entities/payment';
...
export class UpdatePaymentUseCase {
    ...
    async execute(id: string, updatePaymentDto: UpdatePaymentDto): Promise<Payment> {
        ...
        if (updatePaymentDto.reference !== undefined) {
            const paymentWithSameReference = await this.paymentRepository.findByReference(updatePaymentDto.reference);
            if (paymentWithSameReference && paymentWithSameReference.id !== id) {
                throw new BadRequestException('Payment with this reference already exists');
            }
        }
        ...
        if (updatePaymentDto.status !== undefined) {
            UpdatePaymentUseCase.validateStatusTransition(existingPayment.status, updatePaymentDto.status);
    ...
    static validateStatusTransition(currentStatus: PaymentStatus, newStatus: PaymentStatus): void {
        switch (newStatus) {
            ...
            case PaymentStatus.USED:
                if (currentStatus !== PaymentStatus.VERIFIED) {
                    throw new BadRequestException('Only verified payments can be marked as used');
                }
                break;
            case PaymentStatus.EXPIRED:
                if (currentStatus !== PaymentStatus.PENDING && currentStatus !== PaymentStatus.VERIFIED) {
                    throw new BadRequestException('Only pending or verified payments can be marked as expired');
                }
                break;
